...
class Queue:
    ...
    def enqueue(self, data):
        #creation of node and link
        node = Node(data)
        #ensure that the front is completely filled up before going to the back
        if self.is_empty():
            self.front = node
        else:
            self.rear.link = node
    ...
    def dequeue(self):
        if self.is_empty():
            ...
            removedata = self.front.data = None #data to remove
